...
Snap=$(aws ec2 describe-images --image-ids $AMI_ID --region us-west-2 --query 'Images[*].BlockDeviceMappings[*].Ebs.SnapshotId' --output text)
...
for SNAPSHOT in $Snap; do aws ec2 delete-snapshot --region us-east-1 --snapshot-id $SNAPSHOT; done
if [ $? -eq 0 ]; then
    echo "Done."
else
